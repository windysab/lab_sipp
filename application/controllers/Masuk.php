...
class Masuk extends CI_Controller
{
	...
	public function index()
	{
		...
		if ($this->input->post('btn') || ($this->input->server('REQUEST_METHOD') === 'GET' && $this->uri->segment(2) == '')) {
			...
			// Get data from model
			$data['datafilter'] = $this->M_Masuk->masuk($jenis_perkara, $lap_bulan, $lap_tahun, $search);

			// Get statistics
			$data['stats'] = $this->M_Masuk->get_statistics($jenis_perkara, $lap_bulan, $lap_tahun, $search);
		}
	...
	public function detail($majelis_id = null)
	{
		...
		$this->load->view('template/new_header');
		$this->load->view('template/new_sidebar');
		$this->load->view('v_masuk_detail', $data);
		$this->load->view('template/new_footer');
	}
	...
	public function export_excel($jenis_perkara = 'Pdt.G', $lap_bulan = null, $lap_tahun = null, $search = null)
	{
		...
		// Get data
		$datafilter = $this->M_Masuk->masuk($jenis_perkara, $lap_bulan, $lap_tahun, $search);
		$stats = $this->M_Masuk->get_statistics($jenis_perkara, $lap_bulan, $lap_tahun, $search);
		...
		$objPHPExcel->getProperties()
			...
			->setTitle("Laporan Perkara Masuk")
			->setSubject("Laporan Perkara Masuk")
			->setDescription("Laporan Perkara Masuk Per Majelis")
			->setKeywords("perkara masuk")
			->setCategory("Laporan");
		...
		$objPHPExcel->getActiveSheet()
			->setCellValue('A1', 'LAPORAN PERKARA MASUK PER MAJELIS')
		...
		foreach ($datafilter as $data) {
			$objPHPExcel->getActiveSheet()
				...
				->setCellValue('C' . $row, $data->masuk)
				->setCellValue('D' . $row, round(($data->masuk / $stats->total_perkara) * 100, 2) . '%');
			$row++;
		}
		...
		$row++;
		$objPHPExcel->getActiveSheet()
			->setCellValue('A' . $row, 'Total Perkara Masuk')
		...
		// Rename worksheet
		$objPHPExcel->getActiveSheet()->setTitle('Perkara Masuk');

		// Set active sheet index to the first sheet, so Excel opens this as the first sheet
		$objPHPExcel->setActiveSheetIndex(0);

		// Set filename
		$filename = "Perkara_Masuk_" . $months[$lap_bulan] . "_" . $lap_tahun . ".xlsx";
	...
	public function export_detail($majelis_id = null)
	{
		...
		// Get detailed cases
		$cases = $this->M_Masuk->get_detail_cases($majelis_id, $jenis_perkara, $lap_bulan, $lap_tahun);
		...
		$objPHPExcel->getProperties()
			->setCreator("SIPP")
			->setLastModifiedBy("SIPP")
			->setTitle("Daftar Perkara Majelis Hakim")
			->setSubject("Daftar Perkara")
			->setDescription("Daftar perkara per majelis hakim")
			->setKeywords("perkara, majelis hakim")
			->setCategory("Laporan");
		...
		// Add header
		$objPHPExcel->getActiveSheet()
			->setCellValue('A1', 'DAFTAR PERKARA MAJELIS HAKIM')
		...
		// Rename worksheet
		$objPHPExcel->getActiveSheet()->setTitle('Daftar Perkara');

		// Set active sheet index
		$objPHPExcel->setActiveSheetIndex(0);

		// Set filename
		$filename = "Daftar_Perkara_" . date('YmdHis') . ".xlsx";
